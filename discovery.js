...
(function(){
  ...
  // Faixa de ferramentas removida na revisão de navegação: salvar virou coração no card, comparar fica na página do produto e o kit na aba Decants.
  ...
  function hydrate(){
    document.querySelectorAll('[data-favorite]').forEach(b=>{const active=favorites.has(b.dataset.favorite);b.setAttribute('aria-pressed',String(active));b.innerHTML=b.classList.contains('fav-icon')?'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20s-7-4.4-7-10a4 4 0 0 1 7-2.6A4 4 0 0 1 19 10c0 5.6-7 10-7 10z" fill="'+(active?'currentColor':'none')+'" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/></svg>':(active?'♥':'♡')+' <span>'+(active?'Salvo':'Salvar')+'</span>';b.setAttribute('aria-label',(active?'Remover ':'Salvar ')+b.dataset.favorite+(active?' dos favoritos':' nos favoritos'));});
    ...
    document.querySelectorAll('[data-compare-open-when]').forEach(el=>el.hidden=compared.size<2);
  }
  function controls(p){return `<div class="discovery-actions"><button type="button" data-favorite="${esc(p.nome)}" aria-pressed="false">♡ Salvar</button>${p.tipo!=='skincare'&&!p.decant&&!ehBodySpray(p)?`<button type="button" data-compare="${esc(p.nome)}" aria-pressed="false">Comparar</button><button type="button" data-open-compare data-compare-open-when hidden>Ver comparação (<span data-compare-count>0</span>)</button>`:''}</div>`;}
  ...
  function renderKit(){
    ...
    let content='';
    if(kitStep===1){content=`<h3>Como quer experimentar?</h3><p class="discovery-intro">Escolha o volume de cada decant e a quantidade de fragrâncias. Cada um vem num vidrinho de decant (borrifador) de ${moeda(DECANT_FRASCO)}, já incluído.</p><fieldset class="kit-options"><legend>Volume por fragrância</legend>${[3,5,10].map(v=>`<label><input type="radio" name="kitVolume" value="${v}" ${v===kitVolume?'checked':''}><span>${v} ml</span></label>`).join('')}</fieldset><fieldset class="kit-options"><legend>Quantas fragrâncias?</legend>${[3,5,10].map(v=>`<label><input type="radio" name="kitTarget" value="${v}" ${v===kitTarget?'checked':''}><span>${v} fragrâncias<small>${Math.round(S.decantRate(v)*100)}% sobre o líquido</small></span></label>`).join('')}</fieldset><p>O desconto não incide sobre os vidrinhos. O total aparece conforme você escolhe.</p><button class="btn btn-gold" data-kit-next>Escolher fragrâncias →</button>`;}
