...
for(const p of all){
  ...
  const direct=wa(available?`Olá, Duna! Tenho interesse no ${full(p)} (${p.decant?'decant de 3 ml':p.tamanho}) — ${S.money(p.preco)} no Pix. Pode confirmar disponibilidade, frete e prazo?`:`Olá, Duna! Gostaria de saber quando ${full(p)} volta ao estoque.`);
  const schema={'@context':'https://schema.org','@type':'Product',name:full(p),description,image:[new URL(p.foto,site).href],brand:{'@type':'Brand',name:p.marca},sku:S.slug(full(p)),url,offers:{'@type':'Offer',url,priceCurrency:'BRL',price:Number(p.preco).toFixed(2),availability:'https://schema.org/'+(available?'InStock':'OutOfStock'),itemCondition:'https://schema.org/NewCondition',description:p.decant?'Preço no Pix para o decant de 3 ml, com o vidrinho de decant incluso. Frete a confirmar.':'Preço no Pix. Frete a confirmar.',seller:{'@type':'Organization',name:'Duna Fragrâncias'}}};
